module QuadTree exposing
    ( BoundingBox, buildBoundingBox, intersectBoundingBoxes, width, height, halfWidth, halfHeight, center
    , Bounded
...
@docs BoundingBox, buildBoundingBox, intersectBoundingBoxes, width, height, halfWidth, halfHeight, center
...
loop start condition updatef return =
    evaluate <|
        loop_ start condition updatef return
...
loop_ start condition updatef return =
    case condition start of
        ...
        False ->
            jump
                (\() ->
                    loop_ (updatef start) condition updatef return
                )
...
{-| Construct a bounding box.
    ...
    buildBoundingBox minX maxX minY maxY

-}
buildBoundingBox : Float -> Float -> Float -> Float -> BoundingBox
buildBoundingBox minX maxX minY maxY =
...
subdivideNE box =
    ...
    in
    buildBoundingBox minX box.horizontal.high minY box.vertical.high
...
subdivideNW box =
    let
        ...
        minY =
            box.vertical.low + halfHeight box
    in
    buildBoundingBox box.horizontal.low maxX minY box.vertical.high
...
subdivideSW box =
    ...
    in
    buildBoundingBox box.horizontal.low maxX box.vertical.low maxY
...
subdivideSE box =
    let
        ...
        maxY =
            box.vertical.high - halfHeight box
    in
    buildBoundingBox minX box.horizontal.high box.vertical.low maxY
        ...
        Leaf box maxSize items ->
            if intersectBoundingBoxes item.boundingBox box then
                let
                    ...
                    insertNew quadrant =
                        Array.foldr (\itemi quadTreei -> insert itemi quadTreei)
...
insertMany items quadTree =
    let
        stoppingCondition param =
            Array.get 0 param.items == Nothing

        loopBody variables =
            case Array.get 0 variables.items of
                Nothing ->
                    variables

                Just item ->
                    { items = Array.slice 1 (Array.length variables.items) variables.items, quadTree = insert item variables.quadTree }
